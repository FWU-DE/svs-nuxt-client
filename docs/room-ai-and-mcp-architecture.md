...
Dieses Dokument liegt im Client-Repo, beschreibt aber beide Seiten. Die Server-Anteile liegen in
`schulcloud-server` (Branch `feat/room-templates-ai`), die Client-Anteile in `nuxt-client`.
Für Details zum MCP-Toolsatz siehe zusätzlich
`schulcloud-server/apps/server/src/modules/mcp-server/README.md`.
...
| Pfad | Rolle |
| --- | --- |
| `src/modules/data/room/roomTemplates/roomTemplates.ts` | Vorlagenkatalog |
| `src/modules/data/room/roomTemplates/types.ts` | Vorlagen- und `Resolved*`-Typen |
| `src/modules/data/room/roomTemplates/roomTemplate.resolver.ts` | Platzhalter und Wiederholungen |
| `src/modules/data/room/roomTemplates/roomTemplates.composable.ts` | Anlegen in drei Durchgängen |
| `src/modules/data/room/roomTemplates/roomAiTemplate.composable.ts` | NDJSON-Stream lesen |
| `src/modules/feature/room/roomTemplates/*.vue` | Galerie, Parameterfelder, Strukturansicht, KI-Eingabe |
| `src/modules/page/room/RoomCreate.page.vue` | die vier Schritte |
